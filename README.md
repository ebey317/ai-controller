# AI Controller — Xbox controller to keyboard/mouse/voice mapping for desktop accessibility
...
AI Controller maps Xbox/PlayStation controller buttons to keyboard, mouse, and voice actions so you can run your entire desktop without leaving the chair. Press a trigger to talk, a button to toggle a floating keyboard, and a stick to move the mouse — all through a single controller.
...
- 🎮 **Universal controller support** — Xbox, PlayStation, DualShock, generic USB, Bluetooth
- 🔌 **Power-loss safe** — systemd user services restart everything on boot
- 📋 **Dictation modes** — PRO, BUBBLY, CASUAL, BOLD, BIG text personality transforms
- 🔧 **Udev rules** — Xbox headset wake and controller USB autosuspend fixes included
...
| Service management | systemd user units | Auto-start, crash recovery, boot persistence |
...
**Supported platforms:** Ubuntu / Mint / Debian (apt), macOS (Homebrew), Windows (WS2L recommended)

## Usage

```bash
# Start everything (push-to-talk + keyboard + legend + voice bridge + AntiMicroX)
bash scripts/start-all.sh

# Start individual components
bash scripts/start-voice-stack.sh     # voice bridge + push-to-talk
bash scripts/start-ui-stack.sh          # keyboard + legend HUD
