...
load_profile() {
    ...
    case "$vendor" in
        "$XBOX_VENDOR")
            profile="$PROFILE_DIR/good_1n.gamecontroller.amgp"
    ...
    # Ensure F13-F18 have X11 keycodes (not in default keymap — needed for ptt_pynput.py and onboard scanner)
    DISPLAY="${DISPLAY:-:0}" xmodmap -e "keycode 202 = F13" 2>/dev/null || true
    DISPLAY="${DISPLAY:-:0}" xmodmap -e "keycode 197 = F14" 2>/dev/null || true
    DISPLAY="${DISPLAY:-:0}" xmodmap -e "keycode 217 = F15" 2>/dev/null || true
    DISPLAY="${DISPLAY:-:0}" xmodmap -e "keycode 219 = F16" 2>/dev/null || true
    DISPLAY="${DISPLAY:-:0}" xmodmap -e "keycode 222 = F17" 2>/dev/null || true
    DISPLAY="${DISPLAY:-:0}" xmodmap -e "keycode 230 = F18" 2>/dev/null || true
