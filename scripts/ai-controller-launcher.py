...
class Launcher(Gtk.Window):
    def __init__(self):
        ...
        self.fix_btn = Gtk.Button(label="Fix Controller + Start Services")
        ...
        # Autostart checkbox — user opts in to launching on boot.
        self.autostart_check = Gtk.CheckButton(label="Start on boot")
        self.autostart_check.set_active(self._autostart_exists())
        self.autostart_check.connect("toggled", self._on_autostart_toggled)
        btn_box.pack_start(self.autostart_check, False, False, 0)
    ...
    AUTOSTART_FILE = os.path.join(AUTOSTART_DIR, "ai-controller-launcher.desktop")
    AUTOSTART_DESKTOP = """[Desktop Entry]
Name=AI Controller
Comment=Start and manage the AI Controller services
Exec={install_dir}/scripts/ai-controller-launcher.sh
    ...
    def _on_autostart_toggled(self, widget):
        if widget.get_active():
            ...
            with open(self.AUTOSTART_FILE, "w") as f:
                f.write(self.AUTOSTART_DESKTOP.format(
                    install_dir=INSTALL_DIR))
            self.status_label.set_text("Autostart on boot: ON")
        else:
            try:
                os.remove(self.AUTOSTART_FILE)
            except FileNotFoundError:
                pass
            self.status_label.set_text("Autostart on boot: OFF")
